...
*	Descrição: 
*
*	Tarefa 02: Cada processo correto executa testes até achar outro processo correto. 
	...
	while (time() < MAX_TIME){
		...
		switch(event) {
			...
			case TEST:
				...
				// Continue procurando ate encontrar processo sem-falha
				for(i = 0; i < N; i++){
					// Encontrou um processo com falha
					if(status(processo[testado].id) != 0) {
						printf("O processo %d encontrou o processo %d FALHO no tempo %5.1f e continuou a procurar\n", token, testado, time());
						
						// Incrementa para testar o proximo, ate achar um sem-falha
						testado++;
					}
