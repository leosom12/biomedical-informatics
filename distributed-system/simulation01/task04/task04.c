...
*	Descrição: 
*
*	Tarefa 04: Quando um processo correto testa outro processo correto obtém as informações de
*	diagnóstico do processo testado sobre todos os processos do sistema exceto aqueles que
		...
		switch(event) {
			...
			case TEST:
				...
				if(status(processo[testado].id) != 0) {
                    ...
                    printf("O valor de STATE do processo %d foi atualizado para 1\n", testado);
